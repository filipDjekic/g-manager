...
@RestController @RequestMapping("/api/v1/machine") @RequiredArgsConstructor public class MachineProtocolController {private final MachineProtocolService service;private final StationConfigurationService configuration;private final StationLeaseService leases;private final StationEnforcementProjectionService enforcement;private final MachinePrincipalProvider principal;private final MachineProtocolObservability metrics;
 @PostMapping("/enroll")public ResponseEntity<EnrollResponse> enroll(@Valid@RequestBody EnrollRequest r){try{return ResponseEntity.status(HttpStatus.CREATED).body(service.enroll(r));}catch(RuntimeException failure){metrics.enrollmentFailed();throw failure;}}
