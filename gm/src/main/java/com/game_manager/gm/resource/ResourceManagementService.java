...
public class ResourceManagementService {
 ...
 public List<ResourceAvailabilityView> availability(UUID areaId,UUID serviceId,Instant start,Instant end){
  ...
  return resources.findByAreaIdOrderByDisplayOrderAscNameAsc(areaId).stream().filter(v->serviceId==null||v.getServiceId().equals(serviceId)).map(v->new ResourceAvailabilityView(v.getId(),v.getAreaId(),v.getServiceId(),v.getCode(),v.getName(),v.getType(),v.getX(),v.getY(),v.getWidth(),v.getHeight(),v.getRotation(),!v.isActive()||!v.isBookable()?ResourceAvailabilityView.Availability.INACTIVE:availability.isResourceAvailable(v.getId(),start,end,null)?ResourceAvailabilityView.Availability.AVAILABLE:ResourceAvailabilityView.Availability.OCCUPIED,start,end)).toList();
 }
 @Transactional @PreAuthorize("hasAuthority('RESOURCE_MANAGE')")
 ...
 @Transactional(readOnly=true) public PhysicalResource requireBookable(UUID id,UUID serviceId){PhysicalResource v=resource(id);if(!v.isActive()||!v.isBookable()||!v.getServiceId().equals(serviceId))throw new ApplicationException(HttpStatus.UNPROCESSABLE_ENTITY,"Selected resource is not bookable for this service");return v;}
 @Transactional public PhysicalResource lockBookable(UUID id,UUID serviceId){PhysicalResource v=resources.findLocked(id).orElseThrow(()->new ApplicationException(HttpStatus.NOT_FOUND,"Resource not found"));if(!v.isActive()||!v.isBookable()||!v.getServiceId().equals(serviceId))throw new ApplicationException(HttpStatus.UNPROCESSABLE_ENTITY,"Selected resource is not bookable for this service");return v;}
