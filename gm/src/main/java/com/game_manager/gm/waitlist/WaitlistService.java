...
public class WaitlistService {
    ...
    public WaitlistResponse join(CreateWaitlistRequest request){AuthenticatedUser customer=customer();CatalogItem service=requireService(request.serviceId());
        ...
        if(availability.isAvailable(request.employeeId(),request.desiredStart(),end,null))throw new ApplicationException(HttpStatus.CONFLICT,"Slot is available and can be reserved directly");
        WaitlistEntry entry=new WaitlistEntry();entry.setCustomerId(customer.id());entry.setEmployeeId(request.employeeId());entry.setServiceId(request.serviceId());entry.setDesiredStart(request.desiredStart());entry.setStatus(WaitlistStatus.WAITING);entry.setActiveKey(activeKey(customer.id(),request));
    ...
    public WaitlistResponse accept(UUID offerId){AuthenticatedUser customer=customer();WaitlistOffer offer=offers.findLocked(offerId).orElseThrow(()->new ApplicationException(HttpStatus.NOT_FOUND,"Waitlist offer not found"));WaitlistEntry entry=offer.getEntry();
        ...
        ReservationResponse reservation=reservations.create(new CreateReservationRequest(offer.getEmployeeId(),entry.getServiceId(),entry.getDesiredStart(),"Waitlist offer"));
    ...
    private String activeKey(UUID customer,CreateWaitlistRequest request){return customer+":"+request.serviceId()+":"+request.employeeId()+":"+request.desiredStart();}
}
