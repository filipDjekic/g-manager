...
public record WaitlistResponse(UUID id, UUID serviceId, UUID employeeId, UUID locationId,
                               UUID resourceId, Instant desiredStart, Instant desiredEnd,
    ...
    public static WaitlistResponse from(WaitlistEntry entry, WaitlistOffer offer) {
        return new WaitlistResponse(entry.getId(), entry.getServiceId(), entry.getEmployeeId(),
                entry.getLocationId(), entry.getResourceId(), entry.getDesiredStart(), entry.getDesiredEnd(), entry.getStatus(), offer == null ? null : offer.getId(),
